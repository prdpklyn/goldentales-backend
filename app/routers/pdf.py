# app/routers/pdf.py
"""
GoldenTales PDF Router
======================
API endpoints for PDF data retrieval and generation.
"""
...
async def get_story_for_pdf(request: GetStoryForPDFRequest) -> StoryForPDFResponse:
    ...
    """
    logger.info(f"Fetching story for PDF: {request.story_id}")
...
async def get_pdf_book_data(story_id: str) -> Dict:
    ...
    """
    logger.info(f"Fetching PDF book data: {story_id}")
    
    try:
        service = get_pdf_data_service()
        book_data = await service.get_pdf_book_data(story_id)
        
        # Return as dict for flexible response
...
async def validate_story_for_pdf(story_id: str) -> Dict:
    ...
    """
    logger.info(f"Validating story for PDF: {story_id}")
    
    try:
        service = get_pdf_data_service()
        
        # Get story data for detailed validation
...
async def get_story_pages(
    ...
    """
    logger.info(f"Fetching pages for story: {story_id}")
